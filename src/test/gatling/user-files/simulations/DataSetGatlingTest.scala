...
/**
 * Performance test for the DataSet entity.
 */
class DataSetGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DataSet entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all dataSets")
            .get("/api/data-sets")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .feed(randomSession)
            .exec(http("Create new dataSet")
            .post("/api/data-sets")
            .headers(headers_http_authenticated)
            .body(StringBody("""${randname}""" )).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_dataSet_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created dataSet")
                .get("${new_dataSet_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created dataSet")
            .delete("${new_dataSet_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
